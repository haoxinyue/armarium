...
function generateTree(list, byIds) {
  ...
  for (let i = 0; i < list.length; i++) {
    //计算到达根节点的路径
    let spath = get2RootPath(list[i], list, byIds).reverse(),
      pNode;
    //填充到树中去
...
export default {
  ...
  effects: {
    *fetch({ payload, cache }, { select, call, put }) {
      ...
      if (payload && payload.hospitalId) {
        response.hospitalId = payload.hospitalId;
      }
      yield put({
        type: 'save',
        payload: response,
  ...
  reducers: {
    save(state, action) {
      ...
      let treeByHospitalIds = {
        ...state.treeByHospitalIds,
      };
      if (action.payload.hospitalId) {
        treeByHospitalIds[action.payload.hospitalId] = tree;
      }
