...
class AreaListSelector extends Component {
    ...
    updateActive(e) {
        ...
        if (target && target.hasAttribute("href")) {
            ...
            let container = document.querySelector(".department-selector .list section")
            container.scrollTop = t.offsetTop - container.offsetTop - 88
        }
    }

    isMatchItem(item, val) {
    ...
    filterList(list, val) {
        let nlist = []
        list.forEach((item) => {
            if (this.isMatchItem(item, val)) {
                nlist.push(item)
            } else {
                let find = item.items && item.items.filter((sitem) => this.isMatchItem.bind(this)(sitem, val));
                if (find && find.length) {
                    nlist.push({
                        ...item,
                        items: find
    ...
    onSearch = (val) => {
        ...
        this.setState({
            inputValue: val,
            dataSource: this.filterList(pd, val)
        });
    }
