/**
 * Created by zhouzechen on 2018/5/3.
 */
...
class DepartmentSelector extends Component {
    ...
    updateActive(e){
        ...
        if(target && target.hasAttribute("href")){
            if(this.lastTarget){
                this.lastTarget.classList.remove("active")
            }
            this.lastTarget = target
            target.classList.add("active")

            let prompt = document.querySelector(".prompt")
            prompt.innerText = target.innerText

            prompt.classList.add("active")

            let t = document.querySelector(target.getAttribute("href"))
            let container =  document.querySelector(".listsection")
            container.scrollTop = t.offsetTop- container.offsetTop
        }
    }
            ...
            <div className="department-selector">
                <div className="list">
                    <section className="listsection">
                        {tData.map((item,i)=> <dl key={item.key}>
                            <dt id={"ds_"+item.key}>{item.name}</dt>
                            {item.items.map((sItem)=><dd data-link={"ds_"+item.key}>{sItem.name}</dd>)}
                        </dl>)}
                    </section>
                </div>

                <nav className="nav-bar" onTouchStart={(e)=>{
                    this.updateActive(e)
                }} onTouchEnd={(e)=>{
                    let prompt = document.querySelector(".prompt")
                ...
                {tData.map((item,v)=> <a key={item.key}
                                         // onTouchMove={(e)=>{
                                         //     let t = document.querySelector("#ds_"+item.key)
                                         //     let container =  document.querySelector(".listsection")
                                         //
                                         //     container.scrollTop = t.offsetTop- container.offsetTop
                                         //     //document.querySelector()
                                         // }}
                                         href={"#ds_"+item.key}>{item.name}</a>)}
                </nav>
...
(function (d, b, a, e) {
    var c = function (g, f) {
        this.$ele = g, this.defaults = {}, this.listBox = provinces, this.options = d.extend(true, this.defaults, f || {})
    };
    c.prototype = {
        constructor: c, init: function () {
            ...
            var f = '<nav id="navBar">', j = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
            for (let i = 0; i < 26; i++) {
            ...
            var f = this, j = a.querySelector("#navBar"), h = null;
            var k = function (o) {
                if (h) {
